...
<script>
	...
	<div class="btn_area a_right">
        <input type="button" class="ubtn primary hover" onclick="fn_edit();" value="저장"/>
        <c:if test="${data.useAt eq '1'}">
			<button type="button" class="ubtn gray03 hover" onclick="fn_addPopupOpen('${data.parntsCmmnCode}', '${data.cmmnCode}');" value="하위 코드 등록"><i class="icon icon_addOnly"></i>하위 코드 등록</button>

        </c:if>
        <c:if test="${data.childCnt < 1}">
			<button type="button" class="ubtn red hover" onclick="fn_delete();" value="삭제"><i class="icon icon_delete"></i>삭제</button>
        </c:if>
    </div>
